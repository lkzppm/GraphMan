...
export const pt: Dictionary = {
  ...
  observatory: {
    ...
    memory: {
      title: 'Memória',
      open: 'Trocar a representação',
      choose: 'Como guardar o grafo?',
      parsed: (vertices: string, edges: string) => `${vertices} vértices, ${edges} arestas`,
      names: { list: 'Lista de adjacência', matrix: 'Matriz de bits', csr: 'CSR' },
      short: { list: 'Lista', matrix: 'Matriz', csr: 'CSR' },
      hints: {
        list: 'um vetor de vizinhos por vértice',
        matrix: 'n² bits, teste de aresta em um passo',
        csr: 'dois arrays, a menor e a mais rápida',
      },
      overBudget: 'além do que uma aba aguenta',
      pick: (name: string) => `Guardar o grafo como ${name}`,
      onTheHeap: (bytes: string) => `${bytes} no heap`,
    },
    couldNotBuild: (name: string, message: string) =>
      `Não foi possível construir a ${name}: ${message}`,
    search: 'Busca',
  ...
  presentation: {
    ...
    slides: {
      ...
      architecture: {
        eyebrow: '01',
        title: 'Arquitetura',
        input: { label: 'entrada', name: 'grafo.txt', hint: 'n, depois uma aresta por linha' },
        normalise: { name: 'EdgeList', hint: 'ordenada, sem laços, sem duplicatas' },
        core: { name: 'graphman', hint: '// um crate Rust, sem dependências no núcleo' },
        reps: [
          { name: 'AdjacencyList', hint: 'O(n + m) palavras' },
          { name: 'AdjacencyMatrix', hint: 'O(n²) bits' },
          { name: 'Csr', hint: 'dois arrays' },
        ],
        algos: ['BFS', 'DFS', 'distância', 'componentes', 'diâmetro'],
        generic: '// escritos uma vez sobre o trait Graph, o usuário escolhe o armazenamento',
        outputs: [
          { name: 'graphman-cli', hint: 'os estudos de caso' },
          { name: 'graphman-wasm', hint: 'o observatório' },
        ],
      },
      decisions: {
        eyebrow: '02',
        title: 'Decisões',
        items: [
          { title: 'Normalizar uma vez', caption: '// laços e duplicatas saem na leitura' },
          { title: 'Buscas observáveis', caption: '// um visitor para a busca no meio do nível' },
          {
            title: 'Nada alocado duas vezes',
            caption: '// o reset só toca o que a última busca acendeu',
          },
          {
            title: 'Orçamento, não crash',
            caption: '// 17,6 GB virou um erro tipado e uma célula',
          },
          { title: 'Diâmetro de quatro jeitos', caption: '// as BFS que cada método gastou' },
        ],
        on: (graph: string) => `no ${graph}`,
      },
      studies: {
        eyebrow: '03',
        title: 'Benchmark',
        ...
        machine: (cpu: string, memory: string) =>
          `${cpu}, ${memory}. Footprint do processo após carregar; média de 100 buscas de raízes distintas, sem leitura nem escrita. ≥ marca um limite inferior.`,
        cta: 'Abrir os estudos de caso',
      },
      observatory: {
        eyebrow: '04',
