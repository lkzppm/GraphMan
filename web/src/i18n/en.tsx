...
export const en = {
  ...
  observatory: {
    ...
    memory: {
      title: 'Memory',
      open: 'Change the representation',
      choose: 'How should it be stored?',
      parsed: (vertices: string, edges: string) => `${vertices} vertices, ${edges} edges`,
      names: { list: 'Adjacency list', matrix: 'Bit matrix', csr: 'CSR' },
      short: { list: 'List', matrix: 'Matrix', csr: 'CSR' },
      hints: {
        list: 'one neighbour vector per vertex',
        matrix: 'n² bits, an edge test in one step',
        csr: 'two arrays, the smallest and the fastest',
      },
      overBudget: 'past what a tab can hold',
      pick: (name: string) => `Store the graph as the ${name}`,
      onTheHeap: (bytes: string) => `${bytes} on the heap`,
    },
    couldNotBuild: (name: string, message: string) => `Could not build the ${name}: ${message}`,
    search: 'Search',
