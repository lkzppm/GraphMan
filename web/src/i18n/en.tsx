  ...
  presentation: {
    ...
    slides: {
      cover: {
        ...
        authors: ['Lucas Pacheco', 'Rogério Santana'],
        stack: ['Rust 2024', 'WebAssembly', 'WebGPU · vgpu', 'Next.js', 'Vercel'],
      },
      architecture: {
        eyebrow: '01',
        title: 'Architecture',
        input: { label: 'input', name: 'grafo.txt', hint: 'n, then one edge per line' },
        normalise: { name: 'EdgeList', hint: 'sorted, no loops, no duplicates' },
        core: { name: 'graphman', hint: '// one Rust crate, no dependencies in the core' },
        reps: [
          { name: 'AdjacencyList', hint: 'O(n + m) words' },
          { name: 'AdjacencyMatrix', hint: 'O(n²) bits' },
          { name: 'Csr', hint: 'two arrays' },
        ],
        algos: ['BFS', 'DFS', 'distance', 'components', 'diameter'],
        generic: '// written once over trait Graph, the user picks the storage',
        outputs: [
          { name: 'graphman-cli', hint: 'the case studies' },
          { name: 'graphman-wasm', hint: 'the observatory' },
        ],
      },
      decisions: {
        eyebrow: '02',
        title: 'Decisions',
        items: [
          { title: 'Normalise once', caption: '// loops and duplicates leave at parse time' },
          { title: 'Observable searches', caption: '// a visitor can stop a search mid-level' },
          {
            title: 'Allocate nothing twice',
            caption: '// a reset touches only what the last search lit',
          },
          {
            title: 'A budget, not a crash',
            caption: '// 17.6 GB becomes a typed error and a table cell',
          },
          { title: 'Diameter, four ways', caption: '// BFS runs each method spent' },
        ],
        on: (graph: string) => `on ${graph}`,
      },
      studies: {
        eyebrow: '03',
        title: 'Benchmark',
        ...
        needed: (bytes: string) => `needs ${bytes}`,
        machine: (cpu: string, memory: string) =>
          `${cpu}, ${memory}. Process footprint after loading; mean of 100 searches from distinct roots, reading and writing excluded. ≥ marks a lower bound.`,
